...
{
    public partial class UstvariStanje : MetroForm
    {
        public UstvariStanje()
        {
            InitializeComponent();
        }
        ...
        private async void MetroButton2_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string test = "{ \"stanje\": \"" + metroTextBox1.Text + "\", \"opis\": \"" + metroTextBox2.Text + "\"}";

                StringContent queryString = new StringContent(test, Encoding.UTF8, "application/json");

                var response = await client.PostAsync("https://equipment-rental.herokuapp.com/cstanja", queryString);
